...
#include "../../include/Algorithms/NMFMissingValueRecovery.h"
...
arma::mat NMFMissingValueRecovery::doNMFRecovery(arma::mat &matrix, uint64_t truncation)
{
	// Copy state 
    ...
    for (uint64_t j = 0; j < input.n_cols; ++j)
    {
        for (uint64_t i : indices[j])
        {
            matrix_new.at(i, j) = input.at(i, j) - shift;
        }
    }
