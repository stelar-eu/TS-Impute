...
#include "../../include/Algorithms/GROUSE.h"

namespace Algorithms
{

arma::mat GROUSE::doGROUSE(arma::mat &input, uint64_t maxrank)
{
	// Copy state 
    ...
    for (uint64_t i = 0; i < input_new.n_cols; ++i)
    {
        indices.emplace_back(arma::find_finite(input_new.col(i)));
    }
    
    arma::mat U = arma::orth(arma::randn<arma::mat>(input_new.n_rows, maxrank));
    
    for (uint64_t outiter = 0; outiter < maxCycles; ++outiter)
    {
        for (uint64_t k = 0; k < input_new.n_cols; ++k)
        {
            // Pull out the relevant indices and revealed entries for this column
            arma::uvec &idx = indices[k];//find(Indicator(:,col_order(k)));
            arma::vec currentCol = input_new.col(k);
            ...
            #if true
            {
                ...
                double t = step_size*sG/(double)( (outiter)*input_new.n_cols + k + 1 );
    ...
    arma::mat R(input_new.n_cols, maxrank);
    ...
    for (uint64_t j = 0; j < input_new.n_cols; ++j)
    {
        ...
        {
            if (std::isnan(input_new.at(i, j)))
            {
                input_new.at(i, j) = recon.at(i, j);
            }
        }
    }
