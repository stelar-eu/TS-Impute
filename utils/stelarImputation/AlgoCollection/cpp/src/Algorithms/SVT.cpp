...
#include "../../include/Algorithms/SVT.h"

namespace Algorithms
{

arma::mat SVT::doSVT(arma::mat &X, double tauScale)
{
	// Copy state 
	arma::mat X_new = X;
    ...
    uint64_t n1 = X_new.n_rows;
    uint64_t n2 = X_new.n_cols;
    ...
    arma::uvec omega = arma::find_finite(X_new);
    ...
    for (uint64_t i = 0; i < n1; ++i)
    {
        ...
        {
            if (!std::isnan(X_new.at(i, j)))
            {
                observed.at(0, counter) = i;
                observed.at(1, counter) = j;
                data[counter] = X_new.at(i, j);
                counter++;
            }
        }
    }
    ...
    arma::vec b = X_new.elem(omega);
    
    arma::sp_mat Y(observed, data, X_new.n_rows, X_new.n_cols);
