...
#include "../../include/Algorithms/SPIRIT.h"

namespace Algorithms
{

arma::mat SPIRIT::doSpirit(arma::mat &A, uint64_t k0, uint64_t w, double lambda)
{
    ...
    for (uint64_t i = 0; i < A_new.n_rows; ++i)
    {
        if (std::isnan(A_new.at(i,0)))
        {
            lastVal = A_new.at(i - 1, 0);
            blockStart = i;
            break;
        }
    }
    for (uint64_t i = blockStart; i < A_new.n_rows; ++i)
    {
        if (std::isnan(A_new.at(i,0)))
        {
            A_new.at(i, 0) = lastVal;
        }
        else
        {
            blockEnd = i;
    ...
    {
        blockEnd = A_new.n_rows - 1;
    }
    
    uint64_t n = A_new.n_cols;
    uint64_t totalTime = A_new.n_rows;
    ...
    for (uint64_t t = 0; t < totalTime; ++t)
    {
        ...
        if (blockStart <= t && t <= blockEnd)
        {
            ...
            A_new.at(t, 0) = xProj[0]; //feed back imputed value
        }
        
        //update W for each y_t
        arma::vec x = A_new.row(t).t();
        ...
        arma::vec Y = subW.t() * A_new.row(t).t(); //project to m-dimensional space
        ...
        arma::vec xActual = A_new.row(t).t(); //actual vector of the current time
