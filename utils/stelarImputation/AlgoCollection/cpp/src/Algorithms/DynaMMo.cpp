...
#include "../../include/Algorithms/DynaMMo.h"
#include "../../include/Algebra/Auxiliary.h"

namespace Algorithms
{

arma::mat DynaMMo::doDynaMMo(arma::mat &X, uint64_t H, uint64_t maxIter, bool FAST)
{
	// Copy state 
	arma::mat X_new = X;
    ...
    uint64_t N = X_new.n_cols;
    uint64_t M = X_new.n_rows;
    ...
    for (uint64_t i = 0; i < X_new.n_rows; ++i)
    {
        non_observed.emplace_back(arma::find_nonfinite(X_new.row(i)));
    }
    
    arma::mat Y;
    
    Algebra::Algorithms::interpolate(X_new, true, &non_observed);
    ...
    while ((iter < maxIter) && (!(isTiny(model.Q0) || isTiny(model.Q) || isTiny(model.R))))
    {
        ...
        std::tie(mu, V, P) = forward(X_new, model, N, H, FAST);
        ...
        MLE_lds(N, M, H, X_new, model, Ez, Ezz, Ez1z);
