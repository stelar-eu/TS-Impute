...
#include "../../include/Algorithms/ROSL.h"
#include "../../include/Algebra/Auxiliary.h"

namespace Algorithms
{

arma::mat ROSL::ROSL_Recovery(arma::mat &input, uint64_t rank, double reg)
{
	// Copy state 
	arma::mat input_new = input;
	
    uint64_t m = input_new.n_rows;
    uint64_t n = input_new.n_cols;
    
    std::vector<arma::uvec> indices;
    
    for (uint64_t i = 0; i < input_new.n_cols; ++i)
    {
        indices.emplace_back(arma::find_nonfinite(input_new.col(i)));
    }
    ...
    Algebra::Algorithms::interpolate(input_new, false, &indices);
