...
from pypots.utils import random
...
def run_gap_generation(ground_truth: Union[str, pd.DataFrame], train_params: dict,
    ...
    if preprocessing:
        ...
        data_gaps = reverse_min_preproc(df=data_gaps, time_column=time_column, index=index)
...
def run_imputation(missing: Union[str, pd.DataFrame], algorithms, params,
    ...
    for alg in algorithms:
        print(f'Executing {alg}')
        new_alg_df = df.copy()
        if alg.lower() in ml_algorithms:
            new_alg_df.loc[:, :] = fill_missing_values_ml(new_alg_df, alg, params[str(alg.lower())], is_multivariate)
        else:
            for cluster in clusters_dict:
                ...
                if df_cl.isnull().any().any():
                    ...
                    if alg.lower() in ['linearimpute', 'meanimpute', 'zeroimpute']:
                        ...
                        new_cluster_alg_df = fill_missing_values(df_cl, alg, params[str(alg.lower())])
        ...
        if preprocessing:
            ...
            new_alg_df = reverse_min_preproc(df=new_alg_df, time_column=time_column, index=index)
...
def run_imputation_ensemble(missing: Union[str, pd.DataFrame], algorithms: dict,
    ...
    if preprocessing:
        ...
        model_pred_df = reverse_min_preproc(df=model_pred_df, time_column=time_column, index=index)
...
def fill_missing_values(time_series: pd.DataFrame, algorithm, var_dict=None):
    ...
    try:
        if algorithm == 'ogdimpute':
            svd: np.ndarray = alg.do_ogd_impute(m_ndarray, var_dict['truncation'])
        elif algorithm == 'rosl':
            svd: np.ndarray = alg.do_rosl(m_ndarray, var_dict['rank'], var_dict['reg'])
        elif algorithm == 'cdmissingvaluerecovery':
            svd: np.ndarray = alg.do_cd_recovery(m_ndarray, var_dict['truncation'], var_dict['eps'])
        elif algorithm == 'grouse':
            m_ndarray = np.transpose(m_ndarray)
            svd: np.ndarray = alg.do_grouse(m_ndarray, var_dict['max_rank'])
        elif algorithm == 'dynammo':
            m_ndarray = np.transpose(m_ndarray)
            svd: np.ndarray = alg.do_dynammo(m_ndarray, var_dict['H'], var_dict['max_iter'], var_dict['fast'])
        elif algorithm == 'iterativesvd':
            svd: np.ndarray = alg.do_iterative_svd(m_ndarray, var_dict['rank'])
        elif algorithm == 'softimpute':
            svd: np.ndarray = alg.do_soft_impute(m_ndarray, var_dict['max_rank'])
        elif algorithm == 'pca_mme':
            m_ndarray = np.transpose(m_ndarray)
            svd: np.ndarray = alg.do_pca_mme(m_ndarray, var_dict['truncation'], var_dict['single_block'])
        elif algorithm == 'linearimpute':
            svd: np.ndarray = alg.do_linear_impute(m_ndarray)
        elif algorithm == 'meanimpute':
            svd: np.ndarray = alg.do_mean_impute(m_ndarray)
        elif algorithm == 'nmfmissingvaluerecovery':
            svd: np.ndarray = alg.do_nmf(m_ndarray, var_dict['truncation'])
        elif algorithm == 'svt':
            svd: np.ndarray = alg.do_svt(m_ndarray, var_dict['tauScale'])
        elif algorithm == 'zeroimpute':
            svd: np.ndarray = alg.do_zero_impute(m_ndarray)
        elif algorithm == 'spirit':
            svd: np.ndarray = alg.do_spirit(m_ndarray, var_dict['k0'], var_dict['w'], var_dict['lambda'])
        elif algorithm == 'tkcm':
            svd: np.ndarray = alg.do_tkcm(m_ndarray, var_dict['truncation'])
        else:
            svd: np.ndarray = alg.do_mean_impute(m_ndarray)
        ...
        print(f'{algorithm} failed! Executing Mean Impute!')
    if c_timeseries.isnull().values.any():
        print(f'After executing {algorithm} we still got missing values! Performing Mean Impute!')
        svd: np.ndarray = alg.do_mean_impute(m_ndarray)
        c_timeseries[c_timeseries.isnull()] = svd
...
def fill_missing_values_ml(time_series: pd.DataFrame, algorithm, var_dict, is_multivariate: bool = False):
    ...
    if 'weight_decay' in var_dict:
        weight_decay = var_dict['weight_decay']
    else:
        weight_decay = 1e-5
    ...
    if algorithm == 'saits':
        ...
        model = SAITS(n_steps=num_timestamps, n_features=num_features, n_layers=n_layers,
                      ...
                      num_workers=num_workers, patience=patience, optimizer=Adam(lr=lr, weight_decay=weight_decay),
                      device=device)
        ...
        model = BRITS(n_steps=num_timestamps, n_features=num_features,
                      ...
                      patience=patience, optimizer=Adam(lr=lr, weight_decay=weight_decay),
                      device=device)
        ...
        model = USGAN(n_steps=num_timestamps, n_features=num_features,
                      ...
                      batch_size=batch_size, num_workers=num_workers,
                      patience=patience, G_optimizer=Adam(lr=lr, weight_decay=weight_decay),
                      D_optimizer=Adam(lr=lr, weight_decay=weight_decay),
                      device=device)
...
def ensemble_model_run_imputation(missing: pd.DataFrame, algorithms,
    ...
    for alg in algorithms:
        ...
        for cluster in clusters_dict:
            ...
            new_cluster_alg_df = pd.DataFrame()
            if alg.lower() in ml_algorithms:
                new_cluster_alg_df = fill_missing_values_ml(df_cl, alg, params[str(alg.lower())], is_multivariate)
...
def reverse_min_preproc(df: pd.DataFrame,
    ...
    :param time_column: the name of the datetime column
    :param index: if true the datetime exists in the index on the original dataframe
    :return: processed dataframe
    """
    if not index:
...
def _cleanup_model(model) -> None:
    """
    Clean up a model from memory to free up GPU and system resources.

    :param model: The model object to be deleted and cleaned up.
    """
    del model.model
    del model
    gc.collect()
