...
###################################################################################################################################
############################################# MAIN ZERO SHOT FUNCTIONS ############################################################
###################################################################################################################################


def simple_LLM_main_loop(inference_engine, groq_api_key, llm_method, low_missing_values_combined_lai_df, df_missing, crop_type, prompt_mode, api_url, api_token, starting_date, end_date, model_name, prefix=None):
    """
    Orchestrates the main loop for Zero shot prompting method, to estimate missing LAI values using an LLM.
    ...
    def make_request_simple_LLM_main_loop_ollama(inference_engine, groq_api_key, llm_method, df_missing, serie, missing_date, crop_type, prompt_mode, api_url, api_token, model_name, prefix):
        """
        ...
        if inference_engine == 'groq' or inference_engine == 'ollama_local':
            ...
            #print('messages:', {"input":human_message_content})

            try:
                #response = chain.invoke({"input":messages})
                response = structured_model.invoke(prompt.format(input=human_message_content))
        ...
        ### that is the case of ollama usage that have been installed in a remote server and a proxy is needed as an intermediate step 
            ...
            "temperature": 0.0, 
            'chain_name': 'zero_shot_prompting', 
    ...
    if isinstance(low_missing_values_combined_lai_df, pd.DataFrame):
        ...
        #print('ground truth exists')
    else:
        #print('ground truth didnt provided by the user')
        ...
        if serie != 'time':
            ...
            for idx, missing_date in enumerate(a[b.iloc[:, 1]].values):
                ...
                if (
                    ...
                    try:
                        # Invoke the model
                        llm_response, elapsed_time = make_request_simple_LLM_main_loop_ollama(inference_engine, groq_api_key, llm_method, df_missing, serie, missing_date, crop_type, prompt_mode, api_url, api_token, model_name, prefix)
                        ####################################
                        ...
                        #the alternative when low_df_combined_lai_df.csv is missing 
                        # if isinstance(low_missing_values_combined_lai_df, pd.DataFrame):
