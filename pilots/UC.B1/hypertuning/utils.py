...
def imputation_metrics(input_folder: str, input_file: str, algorithm: str, params: dict):
    ...
    # data scaling
    data_init_scaled, scaler = tsi.dataframe_scaler(df_input=data_init, time_column=data_init.index.name, preprocessing=False,
                                                    index=True)
    ...
    if algorithm.lower() == 'naive':
        ...
        def objective(trial):
            ...
            try:
                ...
                start_time = time.time()
                data_filled = tsi.run_imputation(missing=data_missing, algorithms=algorithms,
                                                 time_column=data_init.index.name, params=parameters, preprocessing=False,
                                                 index=True, is_multivariate=False)[algorithms[0]]
                exec_time = (time.time() - start_time)
